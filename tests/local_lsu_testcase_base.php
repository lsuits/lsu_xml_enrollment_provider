...
abstract class local_xml_testcase_base extends advanced_testcase {
    ...
    protected function create_and_configure_ues(){
        ...
        // config
        set_config('enrollment_provider', 'xml', 'enrol_ues');
    ...
    private function configure_provider(){
        ...
        set_config('student_data',1, 'local_xml');
    ...
    protected function set_datasource_for_stage($datapathSuffix){
        ...
        $this->assertFileExists(static::$datadir.$datapathSuffix);
        set_config('xmldir', static::$datadir.$datapathSuffix, 'local_xml');
        $datadir = get_config('local_xml','xmldir');
        $files = array('INSTRUCTORS.xml', 'STUDENTS.xml', 'SEMESTERS.xml', 'COURSES.xml');
    ...
    public function run_cron_until_step($x=1){
        $s = 1;
        while($s <= $x){
            $this->run_cron_for_step($s);
            $s++;
        }
    }

    public function run_cron_for_step($step = 1){
        $this->set_datasource_for_stage($step);
        $this->ues->cron();

        $this->assertEmpty($this->ues->errors, $this->ues_errors());
    }

    public function ues_errors(){
        return sprintf("UES finished with errors:\n%s", implode("\n-", $this->ues->errors));
    }
}
?>
